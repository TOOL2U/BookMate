// lib/templates/bookmateAppsScriptTemplate.ts

// Optional: silence TS checking in this file, since it's just a big string
// @ts-nocheck

/**
 * Returns the full Google Apps Script template for BookMate,
 * with EXPECTED_SECRET replaced for a specific client.
 */
export function buildBookmateAppsScriptTemplate(secret: string) {
  return BOOKMATE_APPS_SCRIPT_TEMPLATE.replace(
    'REPLACE_WITH_YOUR_WEBHOOK_SECRET',
    secret
  );
}

// NOTE: Paste your entire Apps Script code between the backticks below.
// Do NOT change REPLACE_WITH_YOUR_WEBHOOK_SECRET – it’s our placeholder.
const BOOKMATE_APPS_SCRIPT_TEMPLATE = String.raw`/**
 * Google Apps Script - BookMate Webhook + Dynamic P&L + Inbox + Balance Management
 * Version 9.0 (V9.0) - Master Template (Workspace)
 *
 * This file is the MASTER TEMPLATE for BookMate spreadsheets.
 * When you create a new client spreadsheet based on this template:
 * - You only need to update the EXPECTED_SECRET (if you want a new one).
 * - Sheet/tab names should stay the same for all clients.
 *
 * DEPLOYMENT INSTRUCTIONS (for each spreadsheet copy):
 * 1. Open the Google Sheet → Extensions → Apps Script
 * 2. SELECT ALL existing code and DELETE it
 * 3. COPY this ENTIRE file and PASTE it
 * 4. Set EXPECTED_SECRET below (or reuse same one for all)
 * 5. Click Save (💾)
 * 6. Click Deploy → Manage deployments → Edit (or New deployment)
 * 7. Description: "V9.0 - Transfer Logic with Data!F2 Schema Support"
 * 8. Click Deploy → copy the Web App URL into your webapp env if needed
 *
 * FEATURES:
 * ✨ Transfer logic (neutral category from Data!F2)
 * ✨ P&L via named ranges (month + year)
 * ✨ Inbox listing, delete endpoint
 * ✨ Property/Person breakdown
 * ✨ Overhead expenses breakdown
 * ✨ Bank & cash balances tracking
 * ✨ Named range discovery + fuzzy matching
 */

// ============================================================================
// CONFIGURATION - UPDATE THESE IF NEEDED FOR EACH TEMPLATE / COPY
// ============================================================================

/**
 * Shared secret used by your webapp/mobile app when calling this webhook.
 * - For a single-tenant MVP you can reuse the same secret for all sheets.
 * - For per-client security, generate a unique secret per client and
 *   update it in the webapp + here in that client's copy.
 */
const EXPECTED_SECRET = "REPLACE_WITH_YOUR_WEBHOOK_SECRET";

/**
 * Name of the main transaction sheet (the one where rows are appended).
 * Default for your current BookMate Master Template (Workspace):
 *   'BookMate P&L 2025'
 * If you rename the tab, update this constant.
 */
const SHEET_NAME = 'BookMate P&L 2025';

/**
 * Name of the balances sheet/tab.
 * Default:
 *   'Bank & Cash Balance'
 */
...
/**
 * The header row number in the main transaction sheet where data starts
 * (row 6 in your current template).
 */
const HEADER_ROW = 6; // Data starts from row 6

/**
 * Cache TTL (Time To Live) in seconds for P&L and other cached data.
...
function handleGetOverheadExpensesDetails(period) {
  try {
    ...
    // Get ALL expense categories from Data!B2:B
...
// ============================================================================
// Test functions (safe to keep in master template)
// ============================================================================
...
function testWebhook() {
  const testPayload = {
    ...
    typeOfPayment: "Cash",
    detail: "Test expense from Apps Script V9.0",
    ref: "TEST-V9.0",
    debit: 1000,
    credit: 0
  };
...
function testTransfer() {
  ...
  const transferOut = {
    ...
    typeOfPayment: "Cash - Test",
    detail: "Transfer to Bank A",
    ref: "T-2025-001",
    debit: 500,
    credit: 0
  };
  ...
  const transferIn = {
    ...
    property: "",
    typeOfOperation: "Transfer",
    typeOfPayment: "Bank A",
    detail: "Transfer from Cash - Test",
    ref: "T-2025-001",
    debit: 0,
    credit: 500
  };
...
// ============================================================================
// P&L Named Ranges Creation & Verification
// ============================================================================
function createPnLNamedRanges() {
  ...
  const ranges = [
    { name: "Month_Total_Revenue",          cell: monthColumnLetter + "11", description: "This Month Total Revenue (" + currentMonth + ")" },
    { name: "Month_Property_Person_Expense", cell: monthColumnLetter + "20", description: "This Month Total Property or Person Expense (" + currentMonth + ")" },
    { name: "Month_Total_Overheads",       cell: monthColumnLetter + "53", description: "This Month Total Overhead Expense (" + currentMonth + ")" },
    { name: "Month_GOP",                   cell: monthColumnLetter + "56", description: "This Month Gross Operating Profit (" + currentMonth + ")" },
    { name: "Month_EBITDA_Margin",         cell: monthColumnLetter + "57", description: "This Month EBITDA Margin (" + currentMonth + ")" },
    { name: "Year_Total_Revenue",          cell: "Q11", description: "Year Total Revenue" },
    { name: "Year_Property_Person_Expense",cell: "Q20", description: "Year Total Property or Person Expense" },
    { name: "Year_Total_Overheads",        cell: "Q53", description: "Year Total Overhead Expense" },
    { name: "Year_GOP",                    cell: "Q56", description: "Year Gross Operating Profit" },
    { name: "Year_EBITDA_Margin",          cell: "Q57", description: "Year EBITDA Margin" }
  ];
  ...
  ranges.forEach(function(rangeConfig) {
    try {
      const cellValue = sheet.getRange(rangeConfig.cell).getValue();
